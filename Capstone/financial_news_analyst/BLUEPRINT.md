...
| **Transparency** | `agent_log` in every response shows the full execution trace (timings, token counts, sentiment). Hallucination score, RAG source chips, and the full RAG Quality Evaluation expander make reasoning visible. `GET /auth/config` exposes auth state to the UI. | `app/api/auth_routes.py`, `app/agents/supervisor_agent.py`, `ui/streamlit_app.py` |
